...
import './RequestDetailPending.css'; // Import the CSS file for styling
import axios from "axios";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faChevronLeft, faFileLines, faPrint } from '@fortawesome/free-solid-svg-icons';
import { faDownload, faCircleCheck, faCircleXmark } from '@fortawesome/free-solid-svg-icons';
...
const RequestDetailPage = () => {
  ...
  return (
    <div className="request-detail-page-pending-version">
      ...
      <div className="big-border">
        ...
        <button className="show-document-button-1"><FontAwesomeIcon icon={faFileLines} /> Show Document</button>
        </div>
        <div className="employee-container-name">
        ...
        <button className="show-document-button"><FontAwesomeIcon icon={faFileLines} className="fa-file"/> Ajouter une note</button>
        </div>
        <div className="request-status">
        <span style={getStatusStyles(request.status)} className="status-request">{request.status}</span>
        </div>
        <div className="approve-reject">
        <button className="approve-button"><FontAwesomeIcon icon={faCircleCheck} className="check"/> Approve</button>
        <button className="reject-button"><FontAwesomeIcon icon={faCircleXmark} className="xmark"/> Reject</button>
        </div>
      </div>
    </div>
  );
